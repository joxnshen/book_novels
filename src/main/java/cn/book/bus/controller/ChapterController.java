...
import cn.book.bus.utils.FileUtils;
...
public class ChapterController {
    ...
    @GetMapping("/download/{fictionId}")
    public File downloadFiction(@PathVariable("fictionId") Integer fictionId){
        StringBuffer fictionStr = new StringBuffer();
        String fictionName = null;
        Fiction fiction = iFictionService.getById(fictionId);
        if(fiction!=null){
            fictionName = fiction.getFictionName();
            String author = fiction.getAuthor();
            fictionStr.append("<H4>"+fictionName + "<H4></br>" + author);
        }
        List<Chapter> chapterList = iChapterService.queryByFictionIdList(fictionId);
        for (Chapter chapter : chapterList) {
            String chapterTitle = chapter.getChapterTitle();
            ChapterContent chapterContent=iChapterContentService.getById(chapter.getContentId());
            fictionStr.append("</br>" + chapterTitle+"</br>"+chapterContent.getContent());
        }

        File file = FileUtils.stringToWrite(fictionStr.toString(), "d:/" + fictionName == null ? "newFile" : fictionName + ".txt");
        return file;
    }
}